...
if not is_authenticated():
    redirect_to_login("pages/admin_system.py")
...
if not refresh_token_if_needed():
    ...
    logout_user(return_to="pages/admin_system.py")
    st.stop()
        ...
        try:
            ...
            if logs:
                ...
                if total_pages > 1:
                    ...
                    with col_prev:
                        st.button("◀️ Previous", disabled=(st.session_state.log_page <= 1), use_container_width=True, on_click=prev_page, key="system_log_prev_btn")
                    
                    with col_info:
                        st.caption(f"Page {st.session_state.log_page} of {total_pages}")
                    
                    with col_next:
                        st.button("Next ▶️", disabled=(st.session_state.log_page >= total_pages), use_container_width=True, on_click=next_page, key="system_log_next_btn")
