-- =====================================================
-- BULK BACKFILL PRE-CONVERTED CURRENCY VALUES
-- =====================================================
-- Single SQL UPDATE that processes all records at once
-- Performance: 1 query vs N queries = 100x+ faster
-- =====================================================

CREATE OR REPLACE FUNCTION backfill_preconverted_values(
    fund_filter VARCHAR(50) DEFAULT NULL
)
RETURNS TABLE(
...
GRANT EXECUTE ON FUNCTION backfill_preconverted_values(VARCHAR) TO service_role;
GRANT EXECUTE ON FUNCTION backfill_preconverted_values(VARCHAR) TO authenticated;

-- Success message
DO $$
BEGIN
    RAISE NOTICE '✅ Backfill pre-converted values function created successfully!';
    RAISE NOTICE '📊 Function: backfill_preconverted_values(fund_filter)';
    RAISE NOTICE '⚡ Performance: Single SQL UPDATE (much faster than Python loop)';
END $$;
