...
@require_admin
def jobs_page():
    """Admin jobs scheduler page (Flask v2)"""
    try:
        from flask_auth_utils import get_user_email_flask
        from user_preferences import get_user_theme
        ...
        logger.debug(f"Rendering jobs page for user: {user_email}")
        ...
        logger.error(f"Error rendering jobs page: {e}", exc_info=True)
        user_theme = 'system'
        nav_context = get_navigation_context(current_page='admin_scheduler')
        return render_template('jobs.html', 
                             user_email='Admin',
...
def _get_cached_jobs_status():
    """Get all jobs status with caching (5s TTL for near real-time)"""
    try:
        from scheduler.scheduler_core import get_all_jobs_status, is_scheduler_running
        from scheduler.jobs import AVAILABLE_JOBS
        
        scheduler_running = is_scheduler_running()
        jobs = get_all_jobs_status() if scheduler_running else []
        
        # Consolidate jobs by base ID (same logic as scheduler_ui.py)
        consolidated_jobs = {}
        for job in jobs:
            job_id = job['id']
            base_id = job_id
            
            # Remove time-based suffixes
        ...
        # Add parameter definitions from AVAILABLE_JOBS
        for job in jobs:
            job_base_id = job.get('actual_job_id', job['id'])
            # Remove suffix to get base job ID for parameter lookup
            base_id_for_params = job_base_id
            for suffix in ['_close', '_open', '_premarket', '_midmorning', '_powerhour', '_postmarket', '_refresh', '_populate', '_collect', '_scan', '_fetch', '_cleanup', '_scrape']:
                if job_base_id.endswith(suffix):
                    base_id_for_params = job_base_id[:-len(suffix)]
                    break
            
            if base_id_for_params in AVAILABLE_JOBS and 'parameters' in AVAILABLE_JOBS[base_id_for_params]:
                job['parameters'] = AVAILABLE_JOBS[base_id_for_params]['parameters']
            else:
                job['parameters'] = {}
        
        # Serialize datetime objects to ISO format strings for JSON response
        # Flask's jsonify handles this automatically, but we ensure consistency
        from datetime import datetime
        for job in jobs:
            if 'next_run' in job and job['next_run'] and isinstance(job['next_run'], datetime):
                job['next_run'] = job['next_run'].isoformat()
            if 'running_since' in job and job['running_since'] and isinstance(job['running_since'], datetime):
                job['running_since'] = job['running_since'].isoformat()
        ...
        logger.error(f"Scheduler module not available: {e}")
        return {
...
def api_jobs_status():
    """Get all jobs status"""
    try:
        data = _get_cached_jobs_status()
        return jsonify(data)
    except Exception as e:
        logger.error(f"Error in api_jobs_status: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 500

@app.route('/api/jobs/run', methods=['POST'])
@require_admin
def api_jobs_run():
    """Run a job immediately with optional parameters"""
    try:
        data = request.get_json()
        ...
        from scheduler.scheduler_core import run_job_now
        
        # Convert date strings to date objects if needed
        processed_params = {}
        for key, value in parameters.items():
            if key in ['target_date', 'from_date', 'to_date'] and isinstance(value, str):
                from datetime import datetime
                try:
                    processed_params[key] = datetime.fromisoformat(value).date()
                except ValueError:
        ...
        if success:
            ...
            return jsonify({"error": "Failed to start job. Check logs for details."}), 500
    except Exception as e:
        logger.error(f"Error in api_jobs_run: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 500
...
def api_jobs_pause():
    ...
    try:
        ...
        from scheduler.scheduler_core import pause_job
        ...
        logger.error(f"Error in api_jobs_pause: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 500
...
def api_jobs_resume():
    ...
    try:
        ...
        from scheduler.scheduler_core import resume_job
...
def api_jobs_start_scheduler():
    ...
    try:
        ...
        if result:
            ...
            return jsonify({"error": "Scheduler was already running (or failed to start)"}), 500
    except Exception as e:
        logger.error(f"Error in api_jobs_start_scheduler: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 500
