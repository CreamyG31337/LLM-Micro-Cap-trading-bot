-- Fix: Restore company, sector, industry columns to latest_positions view
-- Description: DF_008 improved daily P&L lookback but removed the securities join.
-- This restores the securities join to get company/sector/industry data.
-- Date: 2025-01-XX
...
-- Update comment
COMMENT ON VIEW latest_positions IS 'Latest position for each ticker with pre-calculated market_value, P&L, and historical 1-day/5-day P&L. Uses flexible lookback to handle weekends/holidays (up to 14 days for daily P&L). Includes company, sector, and industry from securities table.';
