...
with tab_app:
    ...
    try:
        ...
        with col5:
            ...
            search_input = st.text_input("🔍 Search", placeholder="Filter by text...", label_visibility="collapsed", key="log_search_input", value=st.session_state.log_search_text)
            
            # Update session state only when input actually changes
            if search_input != st.session_state.log_search_text:
                st.session_state.log_search_text = search_input
        ...
        search_text = st.session_state.log_search_text
